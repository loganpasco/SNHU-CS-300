...
HashTable::~HashTable() {
    ...
    {
        temp = current; // hold current node 
        current = current->next; // make current node the next node
        delete temp; // delete temp node 
        return;
    }

    nodes.erase(nodes.begin()); //remove beginning nodes 
}
...
void HashTable::Insert(Bid bid) {
    ...
    {
        Node newNode = Node(bid, key); // new node 

        nodes.at(key) = newNode; //add item
    } 
    else
    {
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            ...
            // while loop to make sure the ID is 5 number long 
            while (name.size() < 5) {
                cout << " Try Again Must Be 5 Numbers";
                break;
            }
            ticks = clock();

            bid = bidTable->Search(name);
        ...
        case 4:
			...
			// while loop to make sure the ID is 5 number long 
			while (name.size() < 5) {
                cout << " Try Again Must Be 5 Numbers" << endl;
				cout << "Enter Bid ID: ";
				cin >> name;
			}
            bid = bidTable->Search(name);
            bidTable->Remove(name);
            cout << "Bid "; displayBid(bid); 
